...
export function PlaylistEditor({ playlist: initial }: PlaylistEditorProps) {
  ...
  const handleSave = async () => {
    setSaving(true);
    await updatePlaylist(initial.id, { name, description });
    setSaving(false);
    router.refresh();
  };
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div className="flex-1 space-y-3">
          <input
            ...
            onBlur={handleSave}
            className="w-full max-w-lg bg-transparent text-3xl font-bold focus:outline-none focus:ring-2 focus:ring-accent rounded px-1"
          />
          <textarea
            value={description}
            ...
            rows={2}
            className="w-full max-w-lg resize-none bg-transparent text-sm text-text-muted focus:outline-none focus:ring-2 focus:ring-accent rounded px-1"
          />
        </div>
